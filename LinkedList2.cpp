...
//单链表中，每个节点都包括两部分内容：数据域和指向下一个节点的指针
struct ListNode {
    string val;        // 数据域为字符串类型
    ListNode *next;    // 指向下一个节点的指针
    ListNode(string x) : val(x), next(nullptr) {}    // 节点构造函数，将x传入val，该节点的 next 指针初始化为 NULL。
};
...
public:
    ...
    ListNode* p = head;
        ...
        head = nullptr;  // 将头指针置为空指针，避免悬空指针的问题
    }
    ...
    void insertNode(string str) {
        ...
        while (p->next != nullptr) {// 1st insert p->next == null ; 2nd insert 开始 逐个往后找node
            p = p->next;
        }
    ...
    void deleteNode(string str) {
        ...
        while (p != nullptr) {         //TODO：测试0522 如果删除头节点会怎么样？
...
int main() {
    ...
    list.deleteNode("");// TODO：0522 删除head节点test
    ////删除成功
